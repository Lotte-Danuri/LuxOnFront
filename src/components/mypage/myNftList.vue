      ...
      <ul class="product__list thum--4 gap--mid">
        <li class="product__item" v-for="product in state.products" v-bind:key="product">
          <div class="product__thum gray">
            <router-link :to="{
              name: 'nft',
              params: { userId: state.userInfo.id, productId: product.productId },
            }">
...
export default {
  setup() {
    ...
    const getNftProducts = async()=>{
      try{
        const response = await axios.post('http://43.200.203.135:5000/api/nft/list',{
