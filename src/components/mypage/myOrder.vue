...
export default {
  setup() {
    ...
    const checkNft = async (productId) => {
      try {
        const response = await axios.post('http://43.200.203.135/api/checknft',
          {
            productId: productId,
    ...
    const isNftMinting = async (productId) => {
      try {
        const response = await axios.post('http://43.200.203.135/api/checkmint',
          {
            productId: productId,
          },)
        if (response.data.length == 0) {
    ...
    const publishNft = async (order) => {
      ...
      Swal.fire({
        ...
        preConfirm: () => {
          return axios.post('http://43.200.203.135/api/receipts', {
